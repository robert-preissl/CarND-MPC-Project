 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int i = 0; i < N - 1; i++) {
      ...
      fg[0] += 750 * CppAD::pow(vars[a_start + i], 2);
    }
...
void MPC::Solve(Eigen::VectorXd x0,
                Eigen::VectorXd coeffs,
  ...
  for (int i=0; i <N-1; i++) { // somehow we get out of bounds issues if we iterate til N-1
    ...
    mpc_psi_vals.push_back(   solution.x[psi_start + 1 + i] );
    mpc_v_vals.push_back(     solution.x[v_start + 1 + i]   );
